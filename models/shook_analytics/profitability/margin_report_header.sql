with contract_master as (
    ...
    select 
        ...
        cm.contract_status,
        cm.start_date, 
        coalesce(cm.ACTUAL_CLOSE_DATE, cm.MONTH_CLOSED) as close_date,
        cm.cust_group, 
        cm.customer, 
        cm.pay_terms,
        ...
        round(DIV0((cm.orig_contract_amt - oe.orig_estimate), cm.orig_contract_amt) * 100, 2)  as orig_gross_margin_pct,
        round(DIV0((cm.contract_amt - ce.curr_estimate), cm.contract_amt) * 100, 2)  as curr_gross_margin_pct,
        round(DIV0((cm.contract_amt - pe.proj_estimate), cm.contract_amt) * 100, 2)  as proj_gross_margin_pct,
        round(DIV0(ctd.cost_to_date, ce.curr_estimate) * cm.orig_contract_amt, 2) as earned_revenue,
        cm.billed_amt - (DIV0(ctd.cost_to_date, pe.proj_estimate) * cm.contract_amt) as over_under_billed,
        -- oc.collected_to_date,
        round(ar.ar_balance, 2) as ar_balance,
        -- -- ctd.cost_to_date,
        round(ap.ap_balance, 2) as ap_balance,
        round((cm.billed_amt - ar.ar_balance - cm.CURRENT_RETAIN_AMT) - (ctd.cost_to_date - ap.ap_balance), 2) as cash_flow,
        oe.orig_estimate,
        ce.curr_estimate,
        pe.proj_estimate,
        ctd.cost_to_date,
        pe.proj_estimate - ctd.cost_to_date as cost_to_complete,
        round(ce.curr_estimate - pe.proj_estimate, 0) as over_under,
        -- ctd.committed_cost,
        round(DIV0(ctd.cost_to_date, pe.proj_estimate) * 100, 2) as percent_complete 
    from
